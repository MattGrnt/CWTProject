...
predictions_ENG <- CWT62ENGsplit %>%
  ...
  mutate(
    ...
    # Fits quasi gaussian regression models to each list element using non NA i.e. historic data for COUNT in order to be used to offset the outside_target model
...
#average yearly increase in numbers
CRUKRoundingPerYear(mean(EnglandAnnualFigures$increase, na.rm = TRUE))
```
